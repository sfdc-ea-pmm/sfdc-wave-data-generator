...
def run(batch_id, source_file_name, output_file_name, reference_date=today_datetime, filter_function=None):
    ...
    def get_create_date(values):
        return dateutil.parser.parse(values['CreatedDate'])
    ...
    # add an age column
    data_gen.add_copy_column('Age__c', 'OpportunityAge')
    ...
    # Not a formula. It comes from source file -oktana
    ...
    def create_date_formula(column_values):
        ...
        offset = column_values['OpportunityAge']
        create_date = close_date - timedelta(days=int(offset))
        return create_date.isoformat(sep=' ')
    data_gen.add_formula_column('CreatedDate', create_date_formula)
    ...
    data_gen.add_formula_column('LastActivityDate', formula=last_activity_date_formula)
    ...
    for i in range(row_count):
        ...
        # if create date is before reference date, but the close date is after reference date
            ...
            # ttc = float(column_values['TimeToClose__c']) -oktana
            ttc = float((close_day - reference_date).days) # -oktana
            pct = age / ttc
            ...
            # column_values['SalesStageCount__c'] = ceil(pct * float(column_values['SalesStageCount__c'])) -oktana
